...
#define MIN_SECTOR 0 //if it's cuid card, change it to 0
...
int get_block_number(int blocks, int chk_trail)
{
	...
	do
	{
        ...
        if (blocknr < MIN_SECTOR)  return(0);
...
int key_upd()
{
    ...
    if (i == 2)
    {
        ...
        if ((addr = get_block_number(0xff,0)) < MIN_SECTOR) return(0);
...
int block_action()
{
	...
	if ((addr =get_block_number(max_blocks-1,1)) < MIN_SECTOR)  close_out(0);
...
int change_block_access() 
{
	...
	p_printf (YELLOW,"Max %d. (0 = exit) : ", max_blocks-1);
	if ((addr = get_block_number(max_blocks-1,0)) < MIN_SECTOR ) close_out(0);
